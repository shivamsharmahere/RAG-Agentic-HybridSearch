...
def render_sidebar() -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: Dictionary containing user inputs from the sidebar
            - groq_api_key (str): GROQ API key from input
            - tavily_api_key (str): Tavily API key from input
            - uploaded_files: List of uploaded PDF files
            - chunk_size (int): Selected chunk size for document splitting
            - chunk_overlap (int): Selected chunk overlap
            - process_clicked (bool): Whether process button was clicked
            - llm_selection (str): Selected LLM model name
    """
    with st.sidebar:
        st.title("🧠 Controls")
        ...
        st.session_state.debug_mode = st.checkbox(
            "🔧 Debug Mode", help="Show detailed processing information"
        )
        ...
        env_keys_loaded = bool(st.session_state.get("groq_api_key")) or bool(
        ...
        if uploaded_files:
            ...
            st.info(
                f"📊 {len(uploaded_files)} files selected ({total_size:.1f} MB total)"
            )
        ...
        process_clicked = st.button(
            ...
            help="Process documents to enable Q&A"
            if can_process
            else "Upload files and enter API keys first",
        )
        ...
        if (
            docs is not None
            and len(docs) > 0
            and st.session_state.get("agent_executor")
        ):
            doc_count = len(docs)
...
def render_chat_history():
    ...
    For assistant responses, also shows citations and tool information
