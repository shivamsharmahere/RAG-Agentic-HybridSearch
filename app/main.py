...
def setup_agent(
    ...
    Args:
        ...
        uploaded_files (List[Any]): List of uploaded files from Streamlit
    ...
    with setup_container:
        ...
        try:
            ...
            st.session_state.docs = load_and_chunk_pdfs(
                uploaded_files, chunk_size, chunk_overlap
            )

            if not st.session_state.docs:
                st.error(
                    "❌ No valid documents were processed. Please check your files and try again."
                )
                return
            ...
            st.session_state.faiss_index, st.session_state.bm25_retriever = (
                build_indexes(st.session_state.docs)
            )
            ...
            if not all(
                ...
                for key in ["docs", "faiss_index", "bm25_retriever", "agent_executor"]
            ):
                raise ValueError(
                    "One or more components failed to initialize properly."
                )
...
def main() -> None:
    ...
    loads environment variables, renders the UI, and handles
    user interactions for the RAG system.
    ...
    with col1:
        st.header(APP_TITLE)
        st.caption(
            "💬 Ask questions about your documents • 📋 Request summaries • 🌐 Search the web"
        )
